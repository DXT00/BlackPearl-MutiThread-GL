...
#include <EGL/egl.h>
#include "SharedEGLContextRenderer.h"

#define VERTEX_POS_INDX  0
#define TEXTURE_POS_INDX 1

SharedEGLContextRenderer::SharedEGLContextRenderer()
{
	m_VaoId = GL_NONE;
	m_VboIds[0] = GL_NONE;
...
void SharedEGLContextRenderer::Init()
{
 ...
 void SharedEGLContextRenderer::OnSurfaceChanged(int width, int height) {
     m_ScreenW = width;
     m_ScreenH = height;
     GLRenderLooper::GetInstance()->postMessage(MSG_SurfaceChanged, m_RenderImage.width, m_RenderImage.height);
...
void SharedEGLContextRenderer::Draw()
{
	...
	glBindTexture(GL_TEXTURE_2D, GL_NONE);
	glBindVertexArray(GL_NONE);
    LOGCATE("Finish SharedEGLContextSample::Draw");

}

void SharedEGLContextRenderer::OnAsyncRenderDone(void *callback, int fboTexId) {
	LOGCATE("SharedEGLContextSample::OnAsyncRenderDone callback=%p, fboTexId=%d", callback, fboTexId);
	SharedEGLContextRenderer *ctx = static_cast<SharedEGLContextRenderer *>(callback);
	unique_lock<mutex> lock(ctx->m_Mutex);
	ctx->m_FboTextureId = fboTexId;
	ctx->m_Cond.notify_all();
}

void SharedEGLContextRenderer::UnInit()
{
