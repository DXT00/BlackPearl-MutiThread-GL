...
std::mutex GLRenderLooper::m_Mutex;
...
GLRenderLooper* GLRenderLooper::GetInstance() {
    printf("GLRenderLooper::GetInstance");
...
void GLRenderLooper::OnSurfaceCreated() {
    ...
    {
        printf("GLRenderLooper::OnSurfaceCreated CreateFrameBufferObj fail");
    }
}

void GLRenderLooper::OnSurfaceChanged(int w, int h) {
    printf("GLRenderLooper::OnSurfaceChanged [w,h]=[%d, %d]", w, h);
}
...
void GLRenderLooper::OnSurfaceDestroyed() {
    printf("GLRenderLooper::OnSurfaceDestroyed");
    m_GLEnv->renderDone(m_GLEnv->callbackCtx, m_FboTextureId);
...
bool GLRenderLooper::CreateFrameBufferObj() {
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        printf("RGB2YUYVSample::CreateFrameBufferObj glCheckFramebufferStatus status != GL_FRAMEBUFFER_COMPLETE");
        return false;
    }
    glBindTexture(GL_TEXTURE_2D, GL_NONE);
